<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="auto">
      ...
      <el-form-item label="代理名称" prop="agentId">
        <el-select v-model="queryParams.agentId" style="width: 100%" placeholder="请选择" clearable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['agent:agentproduct:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['agent:agentproduct:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['agent:agentproduct:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['agent:agentproduct:export']"
        >导出</el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column label="代理编号" align="center" prop="agentNo" min-width="100"  sortable show-overflow-tooltip />
      <el-table-column label="代理名称" align="center" prop="agentName" min-width="110"  sortable show-overflow-tooltip />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="auto">
        <el-tabs v-model="activeName">
          <el-tab-pane label="基础信息" name="first">
            <el-form-item label="代理名称" prop="agentId">
              <el-select v-model="form.agentId" style="width: 100%" @change="agentSelectChange" placeholder="请选择" :disabled="!!form.id">
                <el-option
                  v-for="item in agentOptions"
                  :key="item.agentId"
                  :label="item.agentShortName"
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAgentproduct(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        ...
        if (valid) {
          if (this.form.id != null) {
            ...
            addAgentproduct(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除代理支付产品编号为"' + ids + '"的数据项？').then(function() {
        return delAgentproduct(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('agent/agentproduct/export', {
        ...this.queryParams
      }, `agentproduct_${new Date().getTime()}.xlsx`)
    },
    /** 导出按钮操作 */
    agentSelectChange() {
      listSelectProductAgentNoConfig(this.form.agentId).then(response => {
