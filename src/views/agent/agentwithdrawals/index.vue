<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="auto">
      ...
      <el-form-item label="提现金额" prop="realMoney"  v-if="moreSearch" >
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['agent:agentwithdrawals:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['agent:agentwithdrawals:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['agent:agentwithdrawals:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['agent:agentwithdrawals:export']"
        >导出</el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column label="提现金额 " align="center" prop="money" min-width="100" sortable />
      <el-table-column label="提现金额" align="center" prop="realMoney" min-width="100" sortable />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="提现金额" prop="realMoney">
          <el-input v-model="form.realMoney" placeholder="请输入提现金额" />
        </el-form-item>
...
<script>
import { listAgentwithdrawals, getAgentwithdrawals, delAgentwithdrawals, addAgentwithdrawals, updateAgentwithdrawals } from "@/api/agent/agentwithdrawals";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        money: [
          { required: true, message: "提现金额 不能为空", trigger: "blur" }
        ],
        realMoney: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAgentwithdrawals(this.queryParams).then(response => {
        this.agentwithdrawalsList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAgentwithdrawals(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAgentwithdrawals(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAgentwithdrawals(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除代理提现申请编号为"' + ids + '"的数据项？').then(function() {
        return delAgentwithdrawals(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('agent/agentwithdrawals/export', {
        ...this.queryParams
      }, `agentwithdrawals_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
