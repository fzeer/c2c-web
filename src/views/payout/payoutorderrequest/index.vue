    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['payout:payoutorderrequest:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['payout:payoutorderrequest:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['payout:payoutorderrequest:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['payout:payoutorderrequest:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listPayoutorderrequest, getPayoutorderrequest, delPayoutorderrequest, addPayoutorderrequest, updatePayoutorderrequest } from "@/api/payout/payoutorderrequest";

export default {
  name: "Payoutorderrequest",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPayoutorderrequest(this.queryParams).then(response => {
        this.payoutorderrequestList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加上游代付下单请求";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const orderId = row.orderId || this.ids
      getPayoutorderrequest(orderId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改上游代付下单请求";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.orderId != null) {
            updatePayoutorderrequest(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPayoutorderrequest(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除上游代付下单请求编号为"' + orderIds + '"的数据项？').then(function() {
        return delPayoutorderrequest(orderIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('payout/payoutorderrequest/export', {
        ...this.queryParams
      }, `payoutorderrequest_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
