    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['supplier:supplierwalletlog:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" min-width="120" fixed="right">
        <template v-slot="scope">
          <el-button
            ...
            v-hasPermi="['supplier:supplierwalletlog:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['supplier:supplierwalletlog:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listSupplierwalletlog, getSupplierwalletlog, delSupplierwalletlog, addSupplierwalletlog, updateSupplierwalletlog } from "@/api/supplier/supplierwalletlog";

export default {
  name: "Supplierwalletlog",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSupplierwalletlog(this.queryParams).then(response => {
        this.supplierwalletlogList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSupplierwalletlog(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateSupplierwalletlog(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSupplierwalletlog(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除码商资金流水编号为"' + ids + '"的数据项？').then(function() {
        return delSupplierwalletlog(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('supplier/supplierwalletlog/export', {
        ...this.queryParams
      }, `supplierwalletlog_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
